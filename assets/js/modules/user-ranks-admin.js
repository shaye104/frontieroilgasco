...
function parseRobloxRoleIdFromGroupKey(groupKey) {
  const raw = text(groupKey);
  if (!raw) return '';
  if (/^groups\/roles\/\d+$/i.test(raw)) return raw.replace(/\D+/g, '');
  const lower = raw.toLowerCase();
  if (lower.startsWith('roblox_role_path:') || lower.startsWith('roblox_role_id:') || lower.startsWith('roblox_role:')) {
...
function toRobloxRoleGroupKey(roleId) {
  const digits = text(roleId).replace(/\D+/g, '');
  return digits ? `roblox_role_id:${digits}` : '';
}
...
export async function initUserRanksAdmin(config) {
  ...
  const linksRobloxRoleList = document.querySelector('#userRankRobloxRoleLinksList');
  ...
  const robloxRoleLinkForm = document.querySelector('#userRankRobloxRoleLinkForm');
  const groupSelect = document.querySelector('#userRankGroupKeySelect');
  const robloxRoleSelect = document.querySelector('#userRankRobloxRoleSelect');
  ...
  function clearTabTransientState() {
    ...
    if (linksRobloxRoleList) linksRobloxRoleList.innerHTML = '';
    if (permissionsEditor) permissionsEditor.innerHTML = '';
  }

  function renderRobloxRoleSelect() {
    if (!robloxRoleSelect) return;
    const options = ['<option value="">Select Roblox role...</option>'].concat(
      robloxRoles.map((role) => {
        const roleId = text(role?.id || role?.roleId || role?.role_id);
        const roleName = text(role?.displayName || role?.name || role?.description || `Role ${roleId}`);
        return `<option value="${escapeHtml(roleId)}">${escapeHtml(roleName)} (#${escapeHtml(roleId)})</option>`;
      })
    );
    robloxRoleSelect.innerHTML = options.join('');
  }
  ...
  function applyLinksCountSnapshot(rankId, payload) {
    const rank = ranks.find((row) => Number(row.id) === Number(rankId));
    if (!rank) return;
    rank.discord_links_count = Number(Array.isArray(payload?.discordLinks) ? payload.discordLinks.length : 0);
    rank.group_links_count = Number(Array.isArray(payload?.groupLinks) ? payload.groupLinks.length : 0);
    renderRankList();
    if (Number(selectedRankId) === Number(rankId)) {
  ...
  function renderEditorShell() {
    ...
    if (activeTab === 'links') {
      if (robloxRolesLoaded) {
        void loadLinks(rank.id);
      } else {
        void loadRobloxRoles().then(() => loadLinks(rank.id));
      }
    }
  ...
  function renderLinks(rankId, payload) {
    ...
    const robloxRoleLinks = groupLinks.filter((link) => parseRobloxRoleIdFromGroupKey(link?.group_key));
    const websiteGroupLinks = groupLinks.filter((link) => !parseRobloxRoleIdFromGroupKey(link?.group_key));
    const availableGroups = payload?.availableGroups || [];
    const robloxRoleNameById = new Map(
      robloxRoles.map((role) => [text(role?.id), text(role?.displayName || role?.id)])
    );
      ...
      linksDiscordList.querySelectorAll('[data-remove-discord-link]').forEach((button) => {
        button.addEventListener('click', async () => {
          ...
          try {
            ...
            applyLinksCountSnapshot(rankId, nextPayload);
    ...
    if (!linksRobloxRoleList) return;
    if (!robloxRoleLinks.length) {
      linksRobloxRoleList.innerHTML = '<p class="rank-link-empty">No Roblox role mapping configured.</p>';
    } else {
      linksRobloxRoleList.innerHTML = `
        <table class="rank-link-table">
          <thead>
            <tr>
              <th>Role Name</th>
          ...
          <tbody>
            ${robloxRoleLinks
              .map((link) => {
                const roleId = parseRobloxRoleIdFromGroupKey(link.group_key);
                const roleName = robloxRoleNameById.get(roleId) || `Role ${roleId}`;
                return `
                  <tr>
                    <td>${escapeHtml(roleName)}</td>
                    <td><code>${escapeHtml(roleId)}</code></td>
                    <td class="align-right">
                      <button class="btn btn-secondary btn-compact" type="button" data-remove-roblox-role-link="${escapeHtml(link.group_key)}">Remove</button>
                    </td>
                  </tr>
                `;
              })
      ...
      `;
      linksRobloxRoleList.querySelectorAll('[data-remove-roblox-role-link]').forEach((button) => {
        button.addEventListener('click', async () => {
          const groupKey = text(button.getAttribute('data-remove-roblox-role-link'));
          try {
            const nextPayload = await removeUserRankGroupLink(rankId, groupKey);
            linksCache.set(Number(rankId), nextPayload);
            applyLinksCountSnapshot(rankId, nextPayload);
            renderLinks(rankId, nextPayload);
            setFeedback('Roblox role mapping removed.', 'success');
          } catch (error) {
            setFeedback(error.message || 'Unable to remove Roblox role mapping.', 'error', { autoClear: false });
          }
        });
      });
    }
  ...
  async function loadRobloxRoles() {
    try {
      const payload = await getRobloxGroupRoles();
      const roles = Array.isArray(payload?.roles) ? payload.roles : [];
      robloxRoles = roles
        .map((role) => ({
          id: text(role?.id || role?.roleId || role?.role_id),
          displayName: text(role?.displayName || role?.name || role?.description)
        }))
        .filter((role) => role.id)
        .sort((a, b) => {
      ...
      robloxRolesLoaded = true;
    } catch {
      robloxRoles = [];
      robloxRolesLoaded = false;
    }
    renderRobloxRoleSelect();
  }
  ...
  discordLinkForm?.addEventListener('submit', async (event) => {
    ...
    try {
      ...
      discordLinkForm.reset();
      applyLinksCountSnapshot(rank.id, payload);
  ...
  robloxRoleLinkForm?.addEventListener('submit', async (event) => {
    event.preventDefault();
    const rank = selectedRank();
    if (!rank) return;
    const data = new FormData(robloxRoleLinkForm);
    const roleId = text(data.get('robloxRoleId')).replace(/\D+/g, '');
    if (!roleId) {
      setFeedback('Select a Roblox role.', 'error', { autoClear: false });
      return;
    }
    try {
      const existingPayload = linksCache.get(Number(rank.id)) || (await getUserRankLinks(rank.id));
      const existingGroupKeys = (existingPayload?.groupLinks || [])
        .map((row) => text(row?.group_key))
        .filter((groupKey) => parseRobloxRoleIdFromGroupKey(groupKey));
      for (const groupKey of existingGroupKeys) {
        await removeUserRankGroupLink(rank.id, groupKey);
      }
      const payload = await addUserRankGroupLink(rank.id, toRobloxRoleGroupKey(roleId));
      linksCache.set(Number(rank.id), payload);
      applyLinksCountSnapshot(rank.id, payload);
      renderLinks(rank.id, payload);
      setFeedback('Roblox role mapping updated.', 'success');
    } catch (error) {
      setFeedback(error.message || 'Unable to set Roblox role mapping.', 'error', { autoClear: false });
    }
  });
