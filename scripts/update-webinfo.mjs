...
async function buildWebinfo() {
  ...
  const contents = [
    ...
    '- Cloudflare deployment command: `npx wrangler pages deploy` (or `npx wrangler deploy`).',
    '- Static hosting is configured in `wrangler.jsonc` with `pages_build_output_dir` and `assets.directory` both set to `.`.',
