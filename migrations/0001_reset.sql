...
DROP TABLE IF EXISTS finance_collector_remittances;
DROP TABLE IF EXISTS finance_reimbursement_settlements;
...
CREATE TABLE finance_reimbursement_settlements (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  voyage_id INTEGER NOT NULL,
  owner_employee_id INTEGER NOT NULL,
  amount REAL NOT NULL DEFAULT 0,
...
CREATE TABLE finance_collector_remittances (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
...
CREATE INDEX idx_finance_reimbursements_voyage_owner ON finance_reimbursement_settlements(voyage_id, owner_employee_id);
CREATE INDEX idx_finance_reimbursements_owner ON finance_reimbursement_settlements(owner_employee_id);
CREATE INDEX idx_finance_collector_remittances_status ON finance_collector_remittances(status);
CREATE INDEX idx_finance_collector_remittances_collector_status ON finance_collector_remittances(collector_employee_id, status);
...
INSERT OR IGNORE INTO app_permissions (permission_key, permission_group, label, description) VALUES ('voyages.end','voyages','End Voyages','End voyages and finalise voyage accounting.');
INSERT OR IGNORE INTO app_permissions (permission_key, permission_group, label, description) VALUES ('voyages.override','voyages','Voyage Override','Allow force edit/end/delete actions for voyages without skipper assignment.');
