...
export function getRobloxGroupRoles() {
  return requestJson('/api/admin/roblox/group/roles', { method: 'GET', timeoutMs: 15000 });
}

export function getRobloxGroupJoinRequests({ pageSize = 25, pageToken = '' } = {}) {
  const params = new URLSearchParams();
  params.set('pageSize', String(pageSize));
  if (String(pageToken || '').trim()) params.set('pageToken', String(pageToken).trim());
  return requestJson(`/api/admin/roblox/group/join-requests?${params.toString()}`, { method: 'GET', timeoutMs: 15000 });
}

export function acceptRobloxGroupJoinRequest(requesterId) {
...
export function declineRobloxGroupJoinRequest(requesterId) {
...
export function getRobloxGroupMemberships({ userId = '', pageSize = 25, pageToken = '' } = {}) {
  const params = new URLSearchParams();
  params.set('pageSize', String(pageSize));
  if (String(userId || '').trim()) params.set('userId', String(userId).trim());
  if (String(pageToken || '').trim()) params.set('pageToken', String(pageToken).trim());
  return requestJson(`/api/admin/roblox/group/memberships?${params.toString()}`, { method: 'GET', timeoutMs: 15000 });
}

export function updateRobloxGroupMembershipRole(membershipId, { roleName = '', roleId = '' } = {}) {
  return requestJson(`/api/admin/roblox/group/memberships/${encodeURIComponent(String(membershipId || ''))}`, {
    method: 'PATCH',
    timeoutMs: 15000,
...
export function removeRobloxGroupMembership(membershipId) {
  return requestJson(`/api/admin/roblox/group/memberships/${encodeURIComponent(String(membershipId || ''))}/remove`, {
    method: 'POST',
    timeoutMs: 15000
  });
}
...
export function assignEmployeeVessel(employeeId, payload) {
...
export function clearEmployeeVessel(employeeId) {
  return requestJson(`/api/admin/employees/${employeeId}/vessel-assignment`, {
    method: 'DELETE'
  });
}
...
export function listVoyages(options = {}) {
  ...
  const suffix = params.toString() ? `?${params.toString()}` : '';
  return requestJson(`/api/voyages${suffix}`, {
    method: 'GET',
    cacheTtlMs: 0,
    headers: {
      'cache-control': 'no-cache',
...
export function getVoyageOverview(options = {}) {
  ...
  params.set('_rt', String(Date.now()));
  return requestJson(`/api/voyages?${params.toString()}`, {
    method: 'GET',
    cacheTtlMs: 0,
    headers: {
      'cache-control': 'no-cache',
...
export function listFinanceAudit(options = {}) {
  ...
  if (options.settledBy) params.set('settledBy', String(options.settledBy));
  params.set('tzOffsetMinutes', String(clientTzOffsetMinutes()));
  const suffix = params.toString() ? `?${params.toString()}` : '';
...
export function reserveVoyageShipAssignment() {
  return requestJson('/api/voyages/ship-reservation', {
    method: 'POST',
...
export function releaseVoyageShipAssignment(reservationToken) {
  return requestJson('/api/voyages/ship-reservation', {
    method: 'POST',
    body: JSON.stringify({ action: 'release', reservationToken: String(reservationToken || '') }),
    cacheTtlMs: 0
  });
}

export function timeoutVoyageShipAssignment(reservationToken) {
  return requestJson('/api/voyages/ship-reservation', {
    method: 'POST',
    body: JSON.stringify({ action: 'timeout', reservationToken: String(reservationToken || '') }),
    cacheTtlMs: 0
  });
}
...
export function cancelVoyage(voyageId, payload = null) {
  const body = payload && typeof payload === 'object' ? JSON.stringify({ action: 'cancel', ...payload }) : JSON.stringify({ action: 'cancel' });
  return requestJson(`/api/voyages/${voyageId}`, {
    ...
    return requestJson(`/api/voyages/${voyageId}/cancel`, { method: 'POST', body: payload && typeof payload === 'object' ? JSON.stringify(payload) : undefined });
  });
}
...
export function getFleetLeaderboard() {
  ...
  return requestJson(`/api/fleet/leaderboard?${cacheBust}`, {
    method: 'GET',
    cacheTtlMs: 0,
    headers: {
      'cache-control': 'no-cache',
...
export function listShipyardShips(includeInactive = false) {
  const suffix = includeInactive ? '?includeInactive=1' : '';
  return requestJson(`/api/shipyard${suffix}`, {
    method: 'GET',
    cacheTtlMs: 0,
...
export function createShipyardShip(payload) {
  return requestJson('/api/shipyard', {
    method: 'POST',
    body: JSON.stringify(payload || {})
  });
}

export function updateShipyardShip(shipId, payload) {
  return requestJson(`/api/shipyard/${shipId}`, {
    method: 'PUT',
    body: JSON.stringify(payload || {})
  });
}

export function archiveShipyardShip(shipId) {
  return requestJson(`/api/shipyard/${shipId}`, {
    method: 'DELETE'
  });
}
...
export function dismissLiveNotification(notificationId) {
  return Promise.resolve({ ok: true, disabled: true, notificationId: Number(notificationId) || 0 });
}
