...
function renderCartesianLineChart(target, lines, options = {}) {
  ...
  target.innerHTML = `
    <div class="finance-chart-shell">
      <svg class="finance-cartesian-svg" viewBox="0 0 ${width} ${height}" preserveAspectRatio="xMidYMid meet" aria-hidden="true">
...
function renderCollectorRemittances(state) {
  ...
  tbody.innerHTML = rows
    .map((row) => {
      const canManage = Boolean(state.canSettleCollectorRemittances);
      const hasBalance = Number(row.totalAmount || 0) > 0;
      const canSettle = canManage && hasBalance;
