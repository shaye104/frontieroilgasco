...
import { formatLocalDate, formatLocalDateTime } from './local-datetime.js';
...
function extractRequesterId(row) {
  ...
  const requesterPath =
    String(row?.requester?.path || row?.requester?.name || row?.requester?.user?.path || row?.requester || row?.user?.path || row?.user || row?.userPath || '')
      .trim();
...
function joinRequestRequesterLabel(row) {
  ...
  const requesterDisplay = String(
    row?.requester?.displayName ||
      ...
      row?.requester?.name ||
      row?.requester?.user?.displayName ||
      row?.requester?.user?.username ||
      row?.requester?.user?.name ||
...
const DEFAULT_VISIBLE_COLUMNS = ['roblox_username', 'roblox_user_id', 'rank', 'employee_status', 'hire_date'];
const COLUMN_LABELS = {
...
function renderDrawerVesselAssignmentSection(payload, options = {}) {
  ...
  return `
    <article class="drawer-user-groups-card">
      ...
      ${
        current
          ? `<p class="finance-inline-caption">Assigned ${escapeHtml(formatDate(current.assigned_at, true))}</p>`
          : ''
      }
      ${
        canManage
          ? `<form id="drawerVesselAssignForm" class="finance-cashflow-entry-form">
               <div>
          ...
          : '<p class="finance-inline-caption">You do not have permission to manage vessel assignments.</p>'
      }
      ${
        history.length
  ...
  target.innerHTML = `
    ...
    ${
      voyages.length
        ...
        : '<p class="finance-inline-caption">No recent voyages found.</p>'
    }
  `;
}
...
export async function initManageEmployees(config) {
  ...
  function renderJoinRequestsTable() {
    ...
    joinRequestsTableBody.innerHTML = rows
      .map((row) => {
        ...
        return `<tr>
          ...
          <td class="align-right join-request-actions-cell">
            <button class="btn btn-primary btn-compact" type="button" data-join-action="accept" data-requester-id="${escapeHtml(requesterId)}" ${
              state.joinRequestsLoading || !requesterId ? 'disabled' : ''
            }>Accept</button>
            <button class="btn btn-danger btn-compact" type="button" data-join-action="decline" data-requester-id="${escapeHtml(requesterId)}" ${
              state.joinRequestsLoading || !requesterId ? 'disabled' : ''
            }>Decline</button>
          </td>
        </tr>`;
    ...
    try {
      ...
      if (!state.joinRequests.length && joinRequestsFeedback) {
        ...
        showMessage(
          ...
          mode
            ? `No pending join requests found (${mode} mode).`
    ...
    try {
      ...
      const robloxFailureSamples = Array.isArray(diagnostics?.robloxLookupFailedEmployees)
        ? diagnostics.robloxLookupFailedEmployees
            ...
            .join('; ')
        : '';
      const robloxFailureSummary = Number(summary.robloxLookupFailed || 0)
        ? ` Roblox lookup failures are temporary verification failures, not confirmed access issues. Auth mode: ${text(
  ...
  function setDrawerTab(tab) {
    const allowedTabs = new Set(['overview', 'voyages', 'activity', 'access', 'notes', 'disciplinary']);
  ...
  drawer?.addEventListener('submit', (event) => {
    ...
    if (form.id === 'drawerVesselAssignForm') {
      ...
      void (async () => {
        try {
          await assignEmployeeVessel(employeeId, { shipId });
          drawerCache.delete(employeeId);
          state.drawerVesselBusy = false;
          await refreshDrawerData(employeeId, { force: true, tab: state.drawerTab || 'voyages' });
          const feedbackNode = drawer?.querySelector('#drawerVesselAssignmentFeedback');
