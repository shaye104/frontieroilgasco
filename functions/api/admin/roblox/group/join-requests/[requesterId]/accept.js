...
export async function onRequestPost(context) {
  ...
  const result = await callRobloxGroupApi(context.env, `/groups/${cfg.groupId}/join-requests/${requesterId}:accept`, {
    method: 'POST'
  });
  if (!result.ok) {
    return json({ error: 'Failed to accept join request.', details: result.error }, result.status || 502);
  }

  return json({ ok: true, accepted: true, requesterId, groupId: cfg.groupId, raw: result.payload || {} });
}
