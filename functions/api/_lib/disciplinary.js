...
function isSuspendedEmployeeStatus(status) {
  const value = text(status).toUpperCase();
  return value === 'SUSPENDED' || value === 'SUSPENSION' || value.startsWith('SUSP');
}
...
export async function reconcileEmployeeSuspensionState(env, employeeId, actor = null) {
  ...
  if (activeSuspension) {
    ...
    const nextEmployeeStatus = text(activeSuspension.set_employee_status) || 'SUSPENDED';
    ...
    if (hasSuspensionState) {
      ...
      const currentEmployeeStatus = text(employee.employee_status);
      const restoreEmployeeStatus = isSuspendedEmployeeStatus(currentEmployeeStatus) ? 'ACTIVE' : currentEmployeeStatus || null;
      ...
      await env.DB
        ...
        .bind(restoredRank, restoreEmployeeStatus, employeeId)
        .run();
