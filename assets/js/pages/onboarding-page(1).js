...
let canSubmitAllowed = false;

function updateSubmitEnabled() {
  const submitButton = document.querySelector('#submitOnboardingBtn');
  if (!submitButton) return;
  const canSubmit = submitState !== 'SUBMITTING' && canSubmitAllowed && verifyState === 'VERIFIED';
...
function renderMain(bootstrap) {
  ...
  const canEditFlag = Boolean(bootstrap?.uiFlags?.canEditRobloxDetails);
  const canEditByState = state === 'PENDING_PROFILE';
  const canEdit = canEditFlag || canEditByState;
  canSubmitAllowed = Boolean(bootstrap?.uiFlags?.canSubmit) || state === 'PENDING_PROFILE';
...
async function init() {
  ...
  document.querySelector('#onboardingForm')?.addEventListener('submit', async (event) => {
    ...
    if (verifyState !== 'VERIFIED') {
      const verifyResult = await verifyRoblox();
      if (!verifyResult?.verified) {
        showMessage(feedback, verifyResult?.message || 'Verify Roblox details before submitting.', 'error');
        return;
      }
    }
