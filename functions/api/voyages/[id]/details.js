...
import { canManageVoyage, getVoyageBase, getVoyageDetail, requireVoyagePermission, syncVoyageParticipants } from '../../_lib/voyages.js';
...
export async function onRequestPut(context) {
  ...
  if (!canManageVoyage(session, employee, voyage, 'voyages.edit')) {
    return json({ error: 'Only the voyage Officer of the Watch (OOTW) (or voyage override) can edit voyage details.' }, 403);
  }
  ...
  const destinationPort = departurePort;
  ...
  if (!departurePort || !vesselName || !vesselClass || !vesselCallsign) {
  ...
  if (!Number.isInteger(officerOfWatchEmployeeId) || officerOfWatchEmployeeId <= 0) {
    return json({ error: 'Officer of the Watch (OOTW) is required.' }, 400);
  }
  ...
  if (!ports.has(departurePort.toLowerCase())) {
  ...
  if (Number(voyage.officer_of_watch_employee_id) !== officerOfWatchEmployeeId) {
    logs.push(changeLine('Officer of the Watch (OOTW)', oldOow, nextOow));
    if (currentCrewIds.has(officerOfWatchEmployeeId)) {
      logs.push(`Crew updated: ${nextOow} removed because they are Officer of the Watch (OOTW).`);
    }
  }
